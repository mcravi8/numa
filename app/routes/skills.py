...
from app.research.planner import SKILL_MAX_SUBTASKS, build_plan
...
def propose_skill(req: ProposeRequest) -> dict:
    ...
    to edit and then save (POST /skills). id is None to mark it unsaved. The plan
    is capped at the skill limit (8 subtasks)."""
    description = req.description.strip()
    plan = build_plan(description, tickers=[], max_subtasks=SKILL_MAX_SUBTASKS)
    draft = Skill(id=None, name=(description[:80] or "New skill"),
